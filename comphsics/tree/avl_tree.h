...
#ifndef COMPHSICS_ADT_AVL_TREE_H
#define COMPHSICS_ADT_AVL_TREE_H
...
namespace comphsics{
	namespace tree{
		...
		class avl_tree:public bs_tree<DataType,Compare,NodeType>{
		...
		private:
			explicit avl_tree(node_pointer t)
				:bs_tree<DataType,Compare,NodeType>(t){}
			explicit avl_tree(std::nullptr_t):bs_tree<DataType,Compare,NodeType>(nullptr){}
		...
		public:
			...
			avl_tree(const DataType data[],size_t Size):bs_tree<DataType,Compare,NodeType>(nullptr){
				for(size_t Index=0;Index<Size;++Index){	
					insert(data[Index]);
				}
			}
			avl_tree(avl_tree && tree):bs_tree<DataType,Compare,NodeType>((bs_tree<DataType,Compare,NodeType>&&)tree) {}
			// return a shared tree holding the same nodes.
			[[nodiscard("Unused Tree")]] avl_tree make_shared() {
				avl_tree<DataType,Compare,NodeType> Ret(basic_type::_root);
				Ret._owned=false;
			...
			static avl_tree create_avl_tree(std::istream &in=std::cin) {
				avl_tree<DataType,Compare,NodeType> tree(nullptr); 
				char Indicator;
				DataType Data;
			...
			static bool is_balanced(node::b_node<DataType>* root){
				if(!root){
					return true;
				}
			...
			[[nodiscard("Unused Tree")]] static avl_tree create_empty_tree() {
